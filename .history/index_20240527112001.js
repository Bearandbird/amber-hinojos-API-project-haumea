...
fetch(url)
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Example collection ID, replace with actual logic to determine which collection to link to
    const otherCollectionID = 11; // You can use any valid collection ID

    button.addEventListener('click', () => {
        fetch(`https://api.artic.edu/api/v1/collections/${otherCollectionID}`)
            .then(response => response.json())
            .then(data => {
                // Construct the URL to the other collection page
                const collectionUrl = `https://www.artic.edu/collection/${otherCollectionID}`;
                window.location.href = collectionUrl;
            })
            .catch(error => {
                console.error('Error fetching collection data:', error);
                alert('Failed to load the collection. Please try again later.');
            });
    });
});
  .catch((error) => {
